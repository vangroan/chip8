...
pub struct Assembler<'a> {
    ...
    /// Symbol table of labels mapping to their target addresses.
    ///
...
impl<'a> Assembler<'a> {
    ...
    /// Indicates whether any lines have encountered an error.
    ...
    /// Emit two bytes as an instruction.
    ...
    /// Emit a single byte as an instruction.
    ///
    /// Keep in mind that the Chip-8 VM interprets a program
    /// with a 2-byte stride. Ensure that the instructions
    /// are always 2-byte wide.
    fn emit(&mut self, instr: u8) {
        trace!("emit: {instr:02X}");
        self.bytecode.push(instr);
    }

    /// Print the bytecode buffer to stdout.
    #[allow(dead_code)]
...
///
/// This is a second pass which resolves all the forward-declared labels.
impl<'a> Assembler<'a> {
...
impl<'a> Assembler<'a> {
    ...
    const STATEMENT_END: &'static [TK] = &[TK::EOF, TK::Newline];
    ...
    ///
    /// Numbers can be decimal, binary or hexadecimal.
    ...
    /// Parse skip if equal or not equal.
    ///
    /// - `3xnn (SE Vx, byte)`
    /// - `4xnn (SNE Vx, byte)`
    /// - `5xy0 (SE Vx, Vy)`
    ...
    /// Parse skip if key pressed or not pressed.
    ///
    /// - `Ex9E (SKP Vx)`
    /// - `ExA1 (SKNP Vx)`
